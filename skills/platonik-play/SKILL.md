...
2. Lead with the physical world: which creature is carrying light, where stock is waiting, whether construction completed, which home is losing charge, and what route is blocked. For a selected facility, use `industry` for the exact current state or `industry_frames` for an available historical replay tick; use its Rust-derived status, recipe, missing inputs, and remaining work. A current eligible crane transfer is a possibility at that frame, not proof of delivery. Keep hashes, costs, and raw programs available but secondary.
3. Turn the player's wish into one explicit command. Advance with `{"kind":"advance","ticks":32}`, replace one original cell's admitted policy with `{"kind":"set_program","cell":1,"program":{...}}`, open a construction site with `{"kind":"place","structure":"assembler","position":{"x":0,"y":6}}`, or label a facility with `{"kind":"name","facility":92,"name":"East Drill"}`. Stock policies come from `platonik world program <plan>`: `surveyor` (beacon service), `hauler` (deposit gathering, producer collection, frame/part/material/spark supply), and the `upper`/`lower` foundry policies for cell 5. Write each command and resulting world to new files. Never redirect output over the input world.
...
5. Inspect the new world, compare it with the preserved parent when the effect is unclear, and retain failures. The same advance endpoints always replay to the same state and cumulative work. The report's `state.facilities` carry material, spark, part, and frame buffers plus `spent_*` construction/recipe ledgers; `names` maps facility ids to admitted display names. A handoff from a historical replay frame is context: inspect the current revision before applying its requested change.
...
7. Offer one consequential next choice instead of a menu of engine subsystems: improve reliability or throughput, assemble frames or automate transfer, preserve a favorite or change its job, strengthen home or keep the east outpost lit.

The living world has one 24×14 homestead, two light fields and beacons, three material deposits, a working fabricator that consumes one material plus one spark and completes its part after six processing ticks, a storehouse, a declared drill extracting the northeast deposit, and two foundry blueprints with finite material. The agent can grow a two-tier chain by placing an assembler (4 material + 2 parts) that burns 1 material + 1 part + 1 spark over 10 ticks to mint a frame, then a crane (1 material + 1 part + 1 frame) that moves one item every 8 ticks from a lower-ID adjacent ready facility into a higher-ID one. Other bills are storehouse 2 material + 1 part, fabricator 3 material + 2 parts, and drill 2 material + 1 part. Creatures must physically complete every bill; cranes never supply unfinished sites. A drill is the only structure that may sit on a deposit, and it must: place `{"kind":"place","structure":"miner","position":...}` directly on a stock tile. `has_frame`, frame-valued supply/fetch, and `facility_is` let programs operate the chain. Bounds: 4,096 total ticks, 128 recorded events, at most 16 cells and 12 facilities, 1–128 ticks per advance, five facility kinds, two recipes, and buffers of eight. There is no free-form structure system, infinite map, automatic search, or complete Long Trail.

The current stock hauler fetches parts from fabricators and keeps them for construction or assembler inputs. It does not dump parts into a ready storehouse or collect stored parts on its stock route. Custom policies can still use admitted storage operations. Saved worlds carry their own serialized policies; an updated CLI never silently replaces them. When resuming an older world, inspect its actual program and preserve the original revision before proposing a policy change.

### Use the optional Algal planner

The ordinary workflow above does not need Algal execution: this agent can still author a command and use `world act`. When the player explicitly wants a model-backed proposal, inspect the fixed planner with `world organism`, then run `world propose <current.world.json> --host <algal.host.v1.json> --goal '<player wish>' > proposal.json`. A deterministic offline rehearsal uses `--responses <responses.json>` with a response such as `{"planner":{"candidate":"advance-16"}}`. Keep provider credentials in the executor's environment, never in the host file, proposal, world, or chat.

`propose` is read-only. Platonik compiles a compact view and at most 32 valid candidate actions; the model selects an exact candidate ID instead of authoring a command. No new-site candidate is available while any admitted facility remains unfinished, and no policy candidate may remove the last live surveyor or hauler; a constructed child counts only after birth. Inspect the selected `command`, goal, world hash, revision, and Algal receipt before running `world accept <current.world.json> proposal.json > <next.world.json>`. Accept replays the receipt, requires the exact original view and goal, recompiles the same candidates, and applies the selected command through normal validation. A stale, transplanted, malformed, unknown-candidate, or provider-invalid proposal is an operational error, not a world event. Jev typed decisions and command/ACP/Xcb process executors are not admitted on this path. Do not silently fall back to a provider or automatically retry an uncertain external call.
