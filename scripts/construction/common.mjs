...
export function checkedFreeze() {
  const freeze = read(freezeFile), expected = structuredClone(freeze.source);
  const repairFile = "fixtures/evidence/construction-repairs/import-envelope/repair.json";
  if (fs.existsSync(repairFile)) {
    const repair = read(repairFile);
    assert.equal(repair.schema, "platonik-construction-harness-repair-v1");
    assert.equal(repair.freeze_sha256, sha(fs.readFileSync(freezeFile)));
    assert.equal(repair.script_file, "scripts/construction/repair-import-envelope.mjs");
    assert.equal(sha(fs.readFileSync(repair.script_file)), repair.script_sha256);
    assert.deepEqual(repair.changes.map(row => row.file), ["scripts/construction/common.mjs", "scripts/construction/trajectory.mjs"]);
    for (const row of repair.changes) {
      assert.equal(expected.source_files_sha256[row.file], row.before_sha256);
      assert.equal(sha(fs.readFileSync(row.before_file)), row.before_sha256);
      assert.equal(sha(fs.readFileSync(row.after_file)), row.after_sha256);
      expected.source_files_sha256[row.file] = row.after_sha256;
    }
  }
  assert.deepEqual(sourceIdentity(), expected);
  return freeze;
}
