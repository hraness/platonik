...
export const receiptProbeIds = ["duplicate-material", "changed-copy-byte", "skipped-copy-work", "stolen-reservation",
  "premature-wire", "earlier-birth", "changed-child-program", "refunded-copying", "same-tick-child-activation"];
...
export function probes(run, ledger, plan) {
  ...
  for (const [type, ids] of [["receipt", receiptProbeIds], ["bundle", bundleProbeIds]]) for (const id of ids) {
    ...
    try {
      ...
      const result = run.cli(type === "receipt" ? ["verify", input] : ["habitat", "import", input, destination],
