...
const executable=path.resolve("target/release/examples/ark_capacity");assert.equal(sha(fs.readFileSync(executable)),frozen.source.capacity_binary_sha256);
...
const measured=run.cli(plan.capacity.cases.map(id=>`fixtures/evidence/ark-cases/${id}.json`),{executable,metricsPrefix:false,captureRss:true,reservation:124,accepted:[0]});
assert.equal(measured.value.schema,"platonik-ark-capacity-v1");assert.equal(measured.call.metrics.engine_executions,124);assert.equal(measured.value.engine_executions,124);
...
for(const row of coordinator.replays.slice(0,2)) {
 ...
 const verified=run.cli(["habitat","ark",save],{captureRss:true,reservation:32,accepted:[0]});
 assert.deepEqual(verified.value.ark,row.ark);assert.equal(verified.value.habitat.result_hash,row.result_hash);
