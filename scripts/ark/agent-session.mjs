...
if(operation==="candidate") {
  ...
  const entry={sequence:ledger.candidates.length+1,id:null,admission:"submitted",trials:[],control_passed:false,first_call:ledger.calls.length};
  ...
  try {
    ...
    assert.deepEqual(Object.keys(candidate).sort(),["clock_courier","id","payload_courier","selector"]);
    let bundle={clock_courier:candidate.clock_courier,payload_courier:candidate.payload_courier,selector:candidate.selector};
    ...
    if(ledger.owner==="keep"){
      ...
      assert.deepEqual(bundle.clock_courier,reference.clock_courier,"Keep the exact earlier clock courier");
      assert.deepEqual(bundle.payload_courier,reference.payload_courier,"Keep the exact earlier payload courier");
    }
    ...
    entry.control_passed=entry.trials.every(row=>row.control_passed);entry.admission="completed";
  } catch(error) {
  ...
  if(!best)ledger.finished={control_passed:false,reason:"No candidate control_passed all four training worlds; no extra slot or transfer.",logical_cold_runs:ledger.calls.filter(c=>c.args[0]==="run").length};
  ...
  ledger.finished={control_passed:ledger.transfer.every(row=>row.control_passed),all_inputs_retained:true,logical_cold_runs:ledger.calls.filter(c=>c.args[0]==="run").length};
} else assert.equal(operation,"init");
run.save();
console.log(JSON.stringify({owner:ledger.owner,engine_executions:ledger.engine_executions,candidates:ledger.candidates.map(({id,admission,control_passed,work,program_bytes,trials})=>({id,admission,control_passed,work,program_bytes,trials:trials.map(({case_id,control_passed,passed,work,ark})=>({case_id,control_passed,passed,work,ark}))})),selected:ledger.selected?.id??null,transfer:ledger.transfer,finished:ledger.finished??null},null,2));
