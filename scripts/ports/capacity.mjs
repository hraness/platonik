...
const executable=path.resolve("target/release/examples/ports_capacity");assert.equal(sha(fs.readFileSync(executable)),frozen.source.capacity_binary_sha256);
const ledger=createLedger(directory,192,{owner:"capacity",source_digest:identity(frozen.source),platform:process.platform,architecture:process.arch,cpu:os.cpus()[0]?.model??null,
...
const measured=run.cli(plan.capacity.cases.map(id=>`fixtures/evidence/ports-cases/${id}.json`),{executable,metricsPrefix:false,captureRss:true,reservation:124,accepted:[0]});
assert.equal(measured.value.schema,"platonik-ports-capacity-v1");assert.equal(measured.call.metrics.engine_executions,124);assert.equal(measured.value.engine_executions,124);
...
for(const row of coordinator.replays.slice(0,2)) {
 ...
 const save=path.relative(process.cwd(),path.resolve(coordinatorDirectory,row.id,`restored-${plan.replays.find(spec=>spec.id===row.id).restore_ticks.at(-1)}`));
 const verified=run.cli(["habitat","ports",save],{captureRss:true,reservation:32,accepted:[0]});
 assert.deepEqual(verified.value.ports,row.ports);assert.equal(verified.value.habitat.result_hash,row.result_hash);
