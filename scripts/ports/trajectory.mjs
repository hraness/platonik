// Six explicit process advances, with two checked export/import boundaries.
...
export function trajectory(run, id, experiment, { cuts, restoreTicks }) {
  ...
  assert.equal(cuts.length, 6); assert.equal(restoreTicks.length, 2);
  ...
  const ports = run.cli(["habitat", "ports-check", receiptFile], { reservation: 2, accepted: [0] }).value;
  ...
  return { id, first_call, after_call: run.ledger.calls.length,
    ports, commitments_passed: ports.commitments_passed, passed: receipt.result.outcome.passed, work: total(receipt.result.costs),
