...
import {read,identity,protocol,checkedFreeze,caseInput,programs,variant,cold,checkSelection,qualifiedReferences} from "./common.mjs";
...
const ledger=operation==="init"?createLedger(directory,plan.budget.coordinator,{owner:"coordinator",source_digest:identity(freeze.source),references:qualifiedReferences(),comparisons:[],controls:[],replays:[],probes:[],pending_operation:null}):read(path.join(directory,"ledger.json"));
assert.equal(ledger.owner,"coordinator");assert.equal(ledger.source_digest,identity(freeze.source));assert.equal(ledger.maximum_engine_executions,896);
...
if(operation==="comparisons") {assert.equal(ledger.references.length,8);assert(!ledger.comparisons.length);begin();for(const kind of plan.comparison_kinds)for(const id of [...plan.training,...plan.transfer]){ledger.comparisons.push(trial(kind,id));run.save();}}
else if(operation==="controls") {assert.equal(ledger.comparisons.length,8);assert(!ledger.controls.length);begin();for(const {kind,cases} of plan.controls)for(const id of cases){ledger.controls.push(trial(kind,id));run.save();}}
 ...
 if(Object.values(arms).some(arm=>!arm.selected)){ledger.finished={commitments_passed:false,reason:"At least one arm found no complete training solution; no fabricated selected replay."};}
 ...
 ledger.finished={references_commitments_passed:ledger.references.every(row=>row.commitments_passed),all_saved_traces_equal:ledger.replays.every(row=>row.restored_equal&&row.uninterrupted_equal),probes_passed:ledger.probes.every(row=>row.status==="pass"),
  control_findings:Object.fromEntries(plan.control_kinds.map(kind=>[kind,group(kind).every(row=>row.status==="complete"&&!row.commitments_passed)])),logical_cold_runs:ledger.calls.filter(call=>call.args[0]==="run").length};
