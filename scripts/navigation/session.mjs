...
if (operation === "preflight") {
  ...
  assert.equal(id, declared.id, "Consume the declared four draft slots in order");
  ...
  for (const [kind, program] of [["right-wall", courier(original)], ["compact", compact(original)], ["without-memory-writes", ablate(ledger.selected.program)]]) {
...
console.log(JSON.stringify({ operation, engine_executions: ledger.engine_executions,
  ...
  baseline_passes: ["right-wall", "compact", "without-memory-writes"].map(kind => ({ kind, passed: ledger.baselines.filter(row => row.kind === kind && row.passed).length, total: ledger.baselines.filter(row => row.kind === kind).length })),
