...
export const protocol = () => read("fixtures/evidence/navigation-protocol.json");
...
export function fixedSource() {
  ...
  for (const name of ["common.mjs", "recipes.mjs", "session.mjs", "freeze.mjs"])
    files[`scripts/navigation/${name}`] = sha(fs.readFileSync(`scripts/navigation/${name}`));
  ...
  files["fixtures/evidence/navigation-protocol.json"] = sha(fs.readFileSync("fixtures/evidence/navigation-protocol.json"));
...
export function checkedFreeze() {
  const freeze = read("fixtures/evidence/navigation-freeze.json");
...
export function ablate(program) {
  const changed = structuredClone(program);
  for (const rule of changed.rules) {
    rule.remember = null;
    if (rule.action.kind === "write_memory") rule.action = { kind: "wait" };
  }
  return changed;
}
