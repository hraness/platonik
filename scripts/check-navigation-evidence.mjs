// Fresh admission of a finite historical navigation diagnostic, never a search.
...
const protocolFile = `${evidence}navigation-protocol.json`;
const freezeFile = `${evidence}navigation-freeze.json`;
const studyFile = `${evidence}navigation-study.json`;
...
equal(plan.budget, { preflight_engine_executions: 384, study_engine_executions: 2048,
  maximum_logical_cold_runs: 190, maximum_segmented_replays: 4, candidate_slots: 4 });
equal(plan.pause_ticks, [5, 9, 14, 19, 27, 47, 79, 96]);
equal(plan.restoration_tick, 9);
equal(plan.baseline_kinds, ["right-wall", "compact", "without-memory-writes"]);
...
equal(plan.candidates.length, 4);
for (const [index, candidate] of plan.candidates.entries()) {
  equal(candidate.id, `right-run-${index + 1}`);
...
function metrics(ledger, owner, maximum) {
  equal([ledger.schema, ledger.owner, ledger.source_digest], ["platonik-navigation-ledger-v1", owner, study.source_digest]);
...
const temporary = fs.mkdtempSync(path.join(fs.realpathSync(os.tmpdir()), "platonik-navigation-check-"));
...
try {
  ...
  equal(ledger.candidates.length, 4);
  let offset = 0;
  for (const [index, candidate] of ledger.candidates.entries()) {
    ...
    if (candidate.input_unavailable) {
      ...
      equal(candidate.source_file, declaredInput ? declared.source_file : `${evidence}navigation-submissions/rejected-${index + 1}.json`);
  ...
  if (!selected) {
    ...
    console.log(`Checked navigation diagnostic: no all-training-pass candidate; gate remains false. ${receipts.size} fresh receipts, ${executions} admission executions.`);
  } else {
    ...
    const ablated = structuredClone(selected.program);
    for (const rule of ablated.rules) { rule.remember = null; if (rule.action.kind === "write_memory") rule.action = { kind: "wait" }; }
    const programs = { "right-wall": original, compact, "without-memory-writes": ablated };
    ...
    console.log(`Checked navigation study: ${logical} cold attempts, ${receipts.size} distinct receipt byte streams, four fresh restored habitats, ${executions} admission executions; recorded study ${ledger.engine_executions}; navigation gate ${ledger.finished.navigation_gate}.`);
  }
