...
function compressed(kind,raw,maximum=plan.bounds.maximum_bundle_bytes){assert(raw.length<=maximum);const zipped=gzipSync(raw,{level:9});assert(gunzipSync(zipped,{maxOutputLength:maximum}).equals(raw));return put(`fixtures/evidence/answer-${kind}/${sha(raw)}.json.gz`,zipped,{uncompressed_sha256:sha(raw),uncompressed_bytes:raw.length});}
for(const file of ["fixtures/evidence/answer-protocol.json","fixtures/evidence/answer-freeze.json",plan.qualification_file,"fixtures/evidence/answer-calibration.json",...plan.cases.map(row=>row.file)])include(file);
const qualification=read(plan.qualification_file);for(const file of Object.keys(qualification.artifacts))include(file);
...
function receiptFor(key,row,expected){
 ...
 for(const [field,value]of Object.entries({experiment_hash:receipt.experiment_hash,result_hash:receipt.result_hash,status:receipt.result.status,ticks:receipt.result.ticks_completed,passed:receipt.result.outcome.passed,work:total(receipt.result.costs),answered:row.journey.answered}))assert.deepEqual(row[field],value);
 assert.equal(row.journey.experiment_hash,receipt.experiment_hash);assert.equal(row.journey.result_hash,receipt.result_hash);
 assert.equal(row.journey.evidence_hash,identity({kind:"finished",value:receipt.result}));
...
for(const owner of ["keep","frugal"]){
 ...
 for(const candidate of arm.candidates){
  if(candidate.source){const raw=fs.readFileSync(path.join(roots[owner],candidate.source));assert.equal(sha(raw),candidate.source_sha256);candidate.source_bytes=raw.length;if(candidate.trials.length)candidate.source_file=put(`fixtures/evidence/answer-submissions/${owner}-${candidate.sequence}.json`,raw);else{candidate.source_file=null;candidate.source_retained_locally=true;}}
...
const labels={"first-answer":"The first answer","familiar-courier":"Keep my courier","frugal-crew":"Spend less work","old-answer":"An answer from the past"};
for(const [number,row]of ledger.replays.entries()){
 ...
 index.cases.push({id:row.id,label:labels[row.id],detail:row.answered?"The same crew gathers material, builds two members, keeps service running and brings the final spark’s report home.":"Service and a reply are separate claims. This crew keeps sending an old report; a familiar bit cannot prove a new journey.",passed:row.passed,work:row.work,ticks:row.ticks,result_hash:row.result_hash,journey:row.journey,uninterrupted_equal:row.uninterrupted_equal,restored_equal:row.restored_equal,
 cuts:row.checkpoints.map(cut=>{const frame=receipt.result.frames.find(frame=>frame.tick===cut.tick);assert(frame?.complete);assert.equal(identity(frame.state),cut.state_hash);assert.equal(identity(frame.costs),cut.costs_hash);const construction=frame.state.construction,birth=construction?.births.find(item=>item.tick===cut.tick),assembly=construction?.assemblies[0],parent=frame.state.cells.find(cell=>cell.id===5);const returned=row.journey.milestones.find(item=>item.kind==="matching_reply"&&item.tick===cut.tick);const child=frame.activations.find(action=>action.cell===6);const label=cut.tick===receipt.experiment.ticks?"Journey outcome":returned?"The report comes home":birth?.body.cell.id===6?"Reply joins the crew":birth?"Keeper joins the crew":assembly?"A body takes shape":parent?.material!=null?"Material gathered":child?(child.action.kind==="wait"?"Reply waits":child.success?"Reply at work":"Reply tries to send"):"Crew at work";return{tick:cut.tick,label,detail:`${construction?.assemblies.reduce((sum,item)=>sum+item.copied.length,0)??0} bytes in partial bodies; ${construction?.births.length??0} members built; ${frame.state.delivered.length} sparks delivered; ${total(frame.costs)} modeled work.`,state_hash:cut.state_hash,costs_hash:cut.costs_hash};})});
}
if(index.cases.length)put("public/answer/index.json",Buffer.from(JSON.stringify(index)+"\n"));
...
const calibration=ledgers.calibration;assert.deepEqual(JSON.parse(JSON.stringify(calibration,(key,value)=>["stdout_file","stderr_file"].includes(key)?undefined:value)),read("fixtures/evidence/answer-calibration.json"));
const calibrationReceipt=receiptFor("calibration",calibration.replay,caseInput(plan.training[0]));saveArtifacts("calibration",calibration.replay,calibrationReceipt,null);
...
const capacity=ledgers.capacity,raw=fs.readFileSync(path.join(roots.capacity,capacity.measurement));assert.equal(sha(raw),capacity.measurement_sha256);capacity.measurement_file=put("fixtures/evidence/answer-capacity.json",raw);
...
const study={schema:"platonik-answer-study-v1",protocol_sha256:sha(fs.readFileSync("fixtures/evidence/answer-protocol.json")),freeze_sha256:sha(fs.readFileSync("fixtures/evidence/answer-freeze.json")),source_digest:identity(freeze.source),ledger,arms:{keep:ledgers.keep,frugal:ledgers.frugal},calibration,capacity,artifacts,
 finished:{logical_cold_runs,study_engine_executions:ledger.engine_executions+ledgers.keep.engine_executions+ledgers.frugal.engine_executions,qualification_engine_executions:qualification.engine_executions+calibration.engine_executions,capacity_engine_executions:capacity.engine_executions,answered:!!ledger.finished.references_answered&&ledgers.keep.finished.answered&&ledgers.frugal.finished.answered,capacity_passed:capacity.finished.passed,reasoning_tokens:null}};
put("fixtures/evidence/answer-study.json",Buffer.from(JSON.stringify(study)+"\n"));
console.log(JSON.stringify({study:"fixtures/evidence/answer-study.json",finished:study.finished,artifact_files:Object.keys(artifacts).length}));
