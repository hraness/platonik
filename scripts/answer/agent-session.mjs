...
if(operation==="candidate") {
  ...
  const entry={sequence:ledger.candidates.length+1,id:null,admission:"submitted",trials:[],answered:false,first_call:ledger.calls.length};
  ...
  try {
    ...
    assert.deepEqual(Object.keys(candidate).sort(),["builder","courier","id","reply"]);
    let bundle={courier:candidate.courier,builder:candidate.builder,reply:candidate.reply};
    for(const program of Object.values(bundle))assert(program&&Array.isArray(program.rules));
    bundle=normalizePrograms(bundle);
    if(ledger.owner==="keep")assert.deepEqual(bundle.courier,programs(caseInput(plan.training[0])).courier,"Keep the exact earlier courier");
    ...
    entry.answered=entry.trials.every(row=>row.answered);entry.admission="completed";
  } catch(error) {
  ...
  if(!best)ledger.finished={answered:false,reason:"No candidate answered all four training worlds; no extra slot or transfer.",logical_cold_runs:ledger.calls.filter(c=>c.args[0]==="run").length};
  ...
  ledger.finished={answered:ledger.transfer.every(row=>row.answered),all_inputs_retained:true,logical_cold_runs:ledger.calls.filter(c=>c.args[0]==="run").length};
} else assert.equal(operation,"init");
run.save();
console.log(JSON.stringify({owner:ledger.owner,engine_executions:ledger.engine_executions,candidates:ledger.candidates.map(({id,admission,answered,work,program_bytes,trials})=>({id,admission,answered,work,program_bytes,trials:trials.map(({case_id,answered,passed,work,journey})=>({case_id,answered,passed,work,journey}))})),selected:ledger.selected?.id??null,transfer:ledger.transfer,finished:ledger.finished??null},null,2));
