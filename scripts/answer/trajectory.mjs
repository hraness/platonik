...
export function trajectory(run, id, experiment, { cuts, restoreTicks }) {
  ...
  const journey = run.cli(["habitat", "answer", receiptFile], { reservation: 2, accepted: [0] }).value;
  ...
  return { id, first_call, after_call: run.ledger.calls.length,
    journey, answered: journey.answered, passed: receipt.result.outcome.passed, work: total(receipt.result.costs),
