...
if(operation==="references") {assert(!ledger.references.length);begin();for(const id of [...plan.training,...plan.transfer]){ledger.references.push(trial("reference",id));run.save();}}
...
else if(operation==="controls") {assert.equal(ledger.comparisons.length,16);assert(!ledger.controls.length);begin();for(const kind of plan.control_kinds)for(const id of plan.training.slice(0,2)){ledger.controls.push(trial(kind,id));run.save();}}
 ...
 if(Object.values(arms).some(arm=>!arm.selected)){ledger.finished={answered:false,reason:"At least one arm found no complete training solution; no fabricated selected replay."};}
 ...
 ledger.finished={references_answered:ledger.references.every(row=>row.answered),all_saved_traces_equal:ledger.replays.every(row=>row.restored_equal&&row.uninterrupted_equal),probes_passed:ledger.probes.every(row=>row.status==="pass"),
  control_findings:Object.fromEntries(plan.control_kinds.map(kind=>[kind,group(kind).every(row=>row.status==="complete"&&!row.answered)])),logical_cold_runs:ledger.calls.filter(call=>call.args[0]==="run").length};
