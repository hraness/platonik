...
import { clampCamera, homeCamera, overviewCamera, resizeCamera, zoomCamera, type MapCamera } from "@/lib/play/map-camera";
...
type MapDrag = { pointerId: number; x: number; y: number; camera: MapCamera; ratio: number; viewportWidth: number; viewportHeight: number; moved: boolean; selection?: Selection };
export function FrontierMap({ report, frame, selection, onSelect, building, target, onTarget, focusPoint, routing, routePoints, onCancel }: {
  report: WorldReport; frame: Frame; selection: Selection; onSelect: (value: Selection) => void;
  building: FacilityKind | null; target: Point; onTarget: (point: Point) => void; focusPoint?: Point; routing: boolean; routePoints: Point[]; onCancel?: () => void;
}) {
  ...
  const [camera, setCamera] = useState<MapCamera>(() => homeCamera(experiment, 1.65, frontier));
  ...
  function clamp(next: MapCamera): MapCamera { return clampCamera(next, experiment, ratio); }
  function cancelDrag(pointerId?: number) {
    const active = drag.current;
    if (!active || (pointerId !== undefined && active.pointerId !== pointerId)) return;
    drag.current = null;
    if (svg.current?.hasPointerCapture(active.pointerId)) svg.current.releasePointerCapture(active.pointerId);
  }
  function home() { cancelDrag(); setHover(undefined); setCamera(homeCamera(experiment, ratio, frontier, viewportWidth < 600)); }
  useEffect(() => {
    ...
    const observer = new ResizeObserver(([entry]) => {
      ...
      const oldRatio = previousRatio;
      previousRatio = nextRatio;
      cancelDrag();
      setRatio(nextRatio);
      setViewportWidth(width);
      setCamera((previous) => oldRatio === undefined
        ? homeCamera(experiment, nextRatio, frontier, width < 600)
        : resizeCamera(previous, experiment, oldRatio, nextRatio));
    });
    observer.observe(viewport.current);
    return () => { observer.disconnect(); cancelDrag(); };
  }, [experiment.width, experiment.height, frontier]);
  useEffect(() => {
    if (focusPoint) {
      cancelDrag();
      setCamera((previous) => clamp({ ...previous, x: focusPoint.x - previous.width / 2, y: focusPoint.y - previous.width / ratio / 2 }));
    }
  }, [focusPoint]);
  ...
  function zoom(factor: number) { cancelDrag(); setCamera((old) => zoomCamera(old, experiment, ratio, factor)); }
  ...
  return <div className="frontier-map-shell">
    <div className="frontier-map" ref={viewport}>
      <svg ref={svg} className={building ? "frontier-canvas is-building" : "frontier-canvas"} viewBox={`${camera.x} ${camera.y} ${camera.width} ${height}`} preserveAspectRatio="none" tabIndex={0} style={{ "--frontier-pixel": `${screenPixel}px` } as CSSProperties}
        role="group" aria-label="Factory world. Drag to explore. Arrow keys pan; plus and minus zoom; Home returns to the settlement. Use the Inspector to inspect machines and couriers with the keyboard."
        onKeyDown={(event) => {
          const directions: Record<string, [number, number]> = { ArrowLeft: [-1, 0], ArrowRight: [1, 0], ArrowUp: [0, -1], ArrowDown: [0, 1] };
          if (directions[event.key]) { event.preventDefault(); cancelDrag(); const [x, y] = directions[event.key]; setCamera((old) => clamp({ ...old, x: old.x + x * 2, y: old.y + y * 2 })); }
          ...
          if (event.key === "Escape") { event.preventDefault(); event.stopPropagation(); cancelDrag(); setHover(undefined); onCancel?.(); }
        }}
        onPointerDown={(event) => {
          if (event.button !== 0 || !event.isPrimary || drag.current) return;
          // Native focus on clipped SVG art can scroll the map and its outer shell.
          event.preventDefault();
          event.currentTarget.focus({ preventScroll: true });
          const hit = (event.target as Element).closest("[data-facility], [data-cell]");
          ...
          drag.current = { pointerId: event.pointerId, x: event.clientX, y: event.clientY, camera, ratio, viewportWidth: rect.width, viewportHeight: rect.height, moved: false, selection: selected };
        ...
        onPointerMove={(event) => {
          ...
          if (active) {
            ...
            if (active.moved) setCamera(clamp({ ...active.camera, x: active.camera.x - dx / active.viewportWidth * active.camera.width, y: active.camera.y - dy / active.viewportHeight * active.camera.width / active.ratio }));
          } else if (building && event.isPrimary) setHover(point(event.clientX, event.clientY));
        }}
        onPointerUp={(event) => {
          const active = drag.current;
          if (!active || event.pointerId !== active.pointerId) return;
          const moved = active.moved || Math.abs(event.clientX - active.x) + Math.abs(event.clientY - active.y) > 5;
          cancelDrag(event.pointerId);
          const rect = event.currentTarget.getBoundingClientRect();
          if (moved || event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) return;
          const position = point(event.clientX, event.clientY);
          if (building || routing) onTarget(position);
          else if (active.selection) onSelect(active.selection);
          else {
        ...
        onPointerCancel={(event) => cancelDrag(event.pointerId)} onLostPointerCapture={(event) => cancelDrag(event.pointerId)} onBlur={() => cancelDrag()} onPointerLeave={() => setHover(undefined)}>
        <defs>
        ...
        {(state.facilities ?? []).map((facility) => {
          ...
          return <g key={facility.id} data-facility={facility.id} className="frontier-entity" aria-hidden="true">
            <rect x={facility.position.x - .4} y={facility.position.y - .65} width="1.8" height="1.8" fill="transparent" />
        ...
        {state.cells.map((cell) => {
          ...
          return <g key={cell.id} data-cell={cell.id} className="frontier-entity" aria-hidden="true">
            <rect x={cell.position.x - .15} y={cell.position.y - .32} width="1.3" height="1.3" fill="transparent" />
