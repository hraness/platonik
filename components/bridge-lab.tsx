...
export function BridgeLab() {
  ...
  useEffect(() => {
    ...
    fetch(`/bridge/${encodeURIComponent(selected)}.receipt.json`, { signal: controller.signal }).then(response => {
  ...
  function chooseCase(id: string) {
    const recorded = index?.cases.find(item => item.id === id);
    if (recorded) setSelected(recorded.id);
  }
  return <>
