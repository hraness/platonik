...
export function ContinuityLab({ index, artifactDirectory = "habitat" }: {
  ...
  artifactDirectory?: "habitat" | "navigation" | "construction" | "answer" | "ark";
}) {
  ...
  return <>
    <div className="lab-field bridge-picker"><label htmlFor="habitat-case">Crew to follow</label><select id="habitat-case" value={selected} onChange={event => select(event.target.value)}>{index.cases.map(item => <option key={item.id} value={item.id}>{item.label} · {artifactDirectory === "answer" ? `${item.journey?.answered ? "answer earned" : "no answer"} · ${item.passed ? "service passed" : "service failed"}` : artifactDirectory === "ark" ? `${item.ark?.control_passed ? "control passed" : "control failed"} · ${item.passed ? "service passed" : "service failed"}` : item.passed ? "mission passed" : "mission failed"}</option>)}</select></div>
    ...
    {error ? <div className="lab-result"><p role="alert">{error}</p><button className="lab-button" onClick={() => setRetry(value => value + 1)}>Retry loading</button></div> : !receipt || !frame ? <p role="status">Loading the recorded habitat…</p> : <>
      <section className="bridge-replay" aria-label="Recorded continuous habitat">
        ...
        <div className="bridge-observations"><h2>What the crew carries forward</h2>
          ...
          <p data-testid="habitat-outcome"><strong>Final {artifactDirectory === "answer" || artifactDirectory === "ark" ? "service" : "mission"}: {chosen.passed ? "passed" : "failed"}.</strong> {chosen.ticks} completed ticks; {number(chosen.work)} modeled work.</p>
        </div>
      </section>
