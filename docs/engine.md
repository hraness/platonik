...
Keep the deterministic interpreter, simulation, checker, and persistence behind distinct module boundaries. The core and browser WASM engine do not depend on Algal; only the native agent-facing CLI does. No service, account, model provider, or Algal executor is needed for the first local episode.
...
Industry v6 charges every facility operation against the state in which it actually executes. A producer completing and making a later crane eligible in the same tick charges both operations. Fuel admission uses that exact sequential work before committing the staged facility phase. An insufficient-fuel result retains work already charged but commits none of that phase's staged effects, including deposit withdrawals. New Copperwake worlds use v6.

Existing v5 histories retain their original behavior: work is estimated from the state before the ordered facility step, so a later crane can start on newly produced output without its separate start charge. Their historical totals and receipt identities are preserved. Opening an old save does not migrate it to v6. V6 also admits up to eight initial material stocks with 256 total units; earlier versions retain their four-stock, 32-unit limits.
...
Use a new output filename for every CLI action. The content-addressed browser URL identifies and contains a compressed history. The browser can continue that world through the same Rust command admission; it saves its current revision locally and exports a portable JSON history. Exchange the latest file or handoff when switching between the browser and an agent, since independent copies do not synchronize automatically.

`platonik world route <world.json> <cell> <waypoints.json|->` compiles a haul route and prints an editable Program without changing the save. Waypoints are a JSON array, for example `[{"x":7,"y":3},{"x":9,"y":3},{"x":9,"y":8},{"x":7,"y":8}]` for the initial freight loop. Apply the returned program through an ordinary `set_program` command. The compiler uses the v6 condition `{"kind":"at_position","position":{"x":7,"y":3}}`, which reads the cell's own tile, plus ordinary heading, cargo, supply, and movement rules. It grants no remote map sensor.

Browser Run submits bounded advances; Pause stops further advances. Placement records an unfinished site and its bill, without moving time or granting supplies. A v6 haul route uses four to eight unique orthogonal turnpoints in a closed, nonintersecting, terrain-clear loop containing the selected original mobile cell and avoiding stationary builders and reserved birthsites. Rust compiles it into ordinary admitted sensor, hauling, turn, and movement rules through `set_program`; it grants no remote navigation or extra movement. A blocked hauler waits for traffic. Selecting a replay tick or inspecting a machine never advances the save. See the [living-world plan](living-world-plan.md) for the current product milestones.
