...
Composition remains a separate design gate. The current Bloom habitat is v4 and the earlier port commitments are v3; their interfaces and world bounds cannot be joined by concatenating JSON. The next credible test needs an explicit adapter or a new v5 fixture whose generated courier is also a port-commitment participant. Until that exists, no integrated campaign or economy claim is admitted.
