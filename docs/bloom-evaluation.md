...
Modeled work counts in-world operations. It is not wall-clock capacity, token cost, or a proof that one program is optimal. The [campaign](campaign.md#the-bloom), [complexity notes](complexity-and-scale.md), and [design validation](design-validation.md) keep those claims separate.

The first bounded agent study now exercises that gate. A `keep` arm and a `frugal` arm each admitted two reference-derived candidates, retained one malformed submission, ran all four training worlds, and transferred the selected program across all four unchanged transfer worlds. Both arms completed 24 engine executions and bloomed in transfer; deterministic scoring selected `mirror-courier` in both because the submitted programs were intentionally equivalent. This establishes that the study protocol, retention rules, and transfer accounting run end to end. It is a harness feasibility result, not evidence that one agent searched better than another. The compact [study record](https://github.com/hraness/platonik/blob/main/fixtures/evidence/bloom-study.json) and compressed arm ledgers preserve the exact calls and receipts.

The fixed workload capacity gate also passes. Thirty measured samples for each of `bloom-left` and `bloom-crossing-right` completed with 124 engine executions total. Median end-to-end sample time was 44.47 ms and 45.33 ms respectively; the largest serialized receipt was 1,258,144 bytes inside the reusable 8 MiB buffer. These are measurements of this bounded Rust workload on the recording machine, not a forecast for a larger ecology. The [capacity record](https://github.com/hraness/platonik/blob/main/fixtures/evidence/bloom-capacity.json) retains every sample.

A fresh [generated-courier exchange](composition-v5-gate.md) now connects variation to a real request and a child-origin acknowledgment in six v4 worlds. It adapts the seed and local controllers, with its own grade, controls, and immutable receipts. Its [capacity gate](exchange-capacity.md) passed on the measured machine and admits a bounded independent-agent study. The earlier v3 two-lane Ports contract remains separate; agent search and the complete campaign still need their own evidence.

As an intermediate check, the two published Bloom replays now carry a reduced one-lane composition record. It binds the selected child’s post-selection confirmation request, depot-origin report, physical service, and preserved loser spare to the same Bloom hashes. This demonstrates provenance continuity inside the v4 habitat; it is deliberately not presented as the v3 port commitment contract. See the [left composition record](https://github.com/hraness/platonik/blob/main/public/bloom/composition/bloom-left.json) and [crossing composition record](https://github.com/hraness/platonik/blob/main/public/bloom/composition/bloom-crossing-right.json).
