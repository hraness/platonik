...
The next decisions follow the field-expedition, continuity, navigation, and functional construction evidence: automatically traverse the compressed campaign with useful constructed execution and continuation, then qualify capacity for that actual workload. Repeat bounded agent search, transfer, replay, recovery, exploit evaluation, and cost measurement after each new capability. Human playtesting comes last, once the complete compressed journey and the relevant automated gates hold. Automated completion establishes reachability and exposes defects; it cannot establish human enjoyment.
