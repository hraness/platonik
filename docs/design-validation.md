...
| Can simple components support more elaborate computation? | The separate signal workbench checks relays, NAND behavior, resettable registers, four-bit addition, and an accumulator | The Rust bridge adds shared spatial transport and control; the v3 constructor copies a prescribed child. A stored-program computer remains unimplemented |
...
The [First Answer diagnostic](answer-evaluation.md) connects physical delivery, two constructed children, and a report matching the final serviced spark in one saved world. Its local ending advances the compressed-campaign work. It does not satisfy the remaining arithmetic, distributed-commitment, endogenous-search, and full chapter-transition contracts. Keep those explicit before declaring the automated campaign gate complete.
...
The next dependency after The First Answer is integrated programmable ark control: admit its arithmetic interface and both service plans under the shared runtime. Then establish distributed commitments and endogenous candidate search before traversing the complete compressed campaign and qualifying that workload. Repeat bounded agent search, transfer, replay, recovery, exploit evaluation, and cost measurement after each new capability. Human playtesting comes last, once the complete compressed journey and the relevant automated gates hold. Automated completion establishes reachability and exposes defects; it cannot establish human enjoyment.
