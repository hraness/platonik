...
The next decisions follow the field-expedition, continuity, and bounded navigation evidence: implement material-dependent construction, then automatically traverse useful constructed execution, continuation, and the compressed campaign. Repeat bounded agent search, transfer, replay, recovery, exploit evaluation, and cost measurement after each new capability. Human playtesting comes last, once the complete compressed journey and the relevant automated gates hold. Automated completion establishes reachability and exposes defects; it cannot establish human enjoyment.
