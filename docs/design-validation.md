...
| Can simple components support more elaborate computation? | The separate signal workbench checks finite circuits; [ark control](ark-evaluation.md) connects physical input, four-bit addition with a five-bit result, two supplied plans, and retained service control in Rust | Repeated computation, integration with construction, and a stored-program computer remain unimplemented |
| Can construction have real limits? | The [v3 construction diagnostic](construction-evaluation.md) passes eight worlds, restored journeys, and removal controls requiring material, activation, and useful ordinary child execution | Multi-part construction, endogenous design, repair, and useful self-construction |
| Does the narrative have an earned end? | The First Answer supplies a checked local construction-to-contact ending; six larger chapter contracts remain specified | Complete chapter-to-chapter execution, meaningful campaign choices, pacing, and an ending players find satisfying |
| Can larger worlds be affordable? | Fixed Rust workload measurements, including the [bridge memory repair](rust-bridge.md#measured-cost-and-the-memory-repair) and [v3 construction measurements](construction-evaluation.md#measured-capacity-and-its-current-limit); a bounded benchmark buffer reduced the measured process peak from 298 to 17.75 MiB | Qualify the actual campaign, larger envelopes, worst-case workloads, supported hardware tiers, and long-lived process behavior |
...
The navigation gate keeps the original event times and requires a frozen courier to complete the preserved failures and declared variations. Construction can begin in a separately declared bounded habitat, but success must require acquired material, admitted offspring, and useful execution by that offspring. It must preserve the existing world and costs across continuation. Only after those capabilities compose should agents attempt the compressed full campaign.

The [navigation diagnostic](navigation-evaluation.md) passes its bounded gate: the selected courier completes all 44 distinct worlds, compared with 14 for the original wall follower and 28 for a compact shuttle. Every noncourier field stays fixed within comparisons; selection uses training only. The failed first search remains counted. This repairs a bounded habit from initialization; it does not admit construction or arbitrary maze recovery.
...
The [port commitments gate](ports-evaluation.md) follows bounded [ark control](ark-evaluation.md) with two finite handoffs under delayed, lost, and duplicate reports. Next establish endogenous candidate search, integrate the admitted capabilities across chapter transitions, and qualify the complete compressed campaign's workload. The present ark performs one calculation; repeatable control and construction must compose within that integrated journey. Repeat bounded agent search, transfer, replay, recovery, exploit evaluation, and cost measurement after each new capability. Human playtesting comes last, once the complete compressed journey and the relevant automated gates hold. Automated completion establishes reachability and exposes defects; it cannot establish human enjoyment.

The [generated-courier exchange](composition-v5-gate.md) now connects bounded Bloom generation and selection to a requested delivery and child-origin acknowledgment in one fresh v4 world. Six references and eight controls establish this adapted one-lane protocol. Its [capacity measurement](exchange-capacity.md) passed the declared execution, verification, memory, and storage limits across 74 pairs and 148 engine executions on the measured machine. This admits a small independent-agent study of that workload. Agent search, the complete compressed campaign, and human playtesting remain later gates.
