...
The next implemented step, [The First Answer](first-answer.md), connects these capabilities into a short construction-to-contact journey. Its [evaluation](answer-evaluation.md) records the local ending, agent comparisons, restored worlds, and workload costs. The complete Long Trail still needs its arithmetic, distributed-commitment, endogenous-search, and chapter-transition contracts. [Human playtesting remains last](design-validation.md#the-automated-path-before-people-play), after the automated path is complete.
