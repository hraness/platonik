...
The crossing witness earns nothing here: `reference:resilient` grafted into the keeper cell fails every switchboard eval case, which is the point — the families measure different skills, and the board shows the negative control honestly instead of hiding it. `season-0001` (crossing) and `season-0002` (crossing + switchboard) are revealed — their manifests publish the salts, so every result re-derives publicly. `season-0003` is open across all three families.
