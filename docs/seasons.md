...
A hosted season is a small operated contest, not a frontier benchmark: the salt lives in a GitHub secret under one maintainer's approval, identity is a GitHub account, and per-entrant sampling means scores carry distribution noise alongside ability. Clearing entries is evidence that a harness produces working programs for worlds it could not have seen — it is not a general intelligence score. Season-0001 evaluates the crossing family only; the evaluator derives salted cases under whichever family an index belongs to, so a future manifest can mix families without a code change. [competition](competition.md) remains the spec for richer families, organism bundles, and prize-bearing rounds.
