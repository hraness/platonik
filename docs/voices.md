...
Cost is a configuration choice, and the frontier model is not the cheapest option — it is the expensive one. Measured through the real request shape (persona plus full ~2k-token digest in the system message, ~400-token reply): `claude-haiku-4.5` costs ~$0.005 per reply, while `alibaba/qwen3.7-flash` — which scored *better* on both batteries — costs **~$0.0002 per reply**, roughly 25× cheaper, because the digest is the dominant input and its per-token price is ~30× lower. The floor measured was `inclusionai/ling-3.0-flash` at ~$0.0001 per reply, but it broke once on a plain outside-knowledge question. A further ~4× input reduction is available if needed: serving only the digest's `facts` and `boundary` (the citeable propositions plus the declared boundary, ~560 tokens) still produced correct cited answers — the `report` prose is redundant with them.
