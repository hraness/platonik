...
export function trajectory(run, id, experiment, { cuts, restoreTicks }) {
  ...
  for (const tick of cuts) {
    ...
    if (restoreTicks.includes(tick)) {
      ...
      assert.deepEqual(restored, report, "Import preserves the complete authoritative report");
  ...
  const checked = run.cli(["verify", receiptFile], { reservation: 2, accepted: [0] }).value;
