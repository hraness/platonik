//! Two fixed eight-cell v4 Bloom inputs, thirty measured repetitions each.
//! This does not qualify larger worlds, additional edits, or the full campaign.
//! Every warmup/sample constructs and independently grades a fresh receipt.
use platonik_core::{bloom, bloom_fixtures, check, model::*, parse_experiment, sim};
...
// This diagnostic requires its two fixed receipts to fit this allocation.
...
fn sample(
    ...
    let start = Instant::now();
    let grade = bloom::grade_receipt(&receipt)?;
    if !grade.bloomed || receipt.result.status != RunStatus::Complete {
        return Err("The exact Bloom capacity workload did not complete its contract.".into());
    }
    ...
    let modeled_work = receipt.result.costs.total();
    let costs = receipt.result.costs.clone();
    ...
    let rss = current_rss();
    let engine_executions = sim::execution_count() - before;
    if engine_executions != 2 {
        return Err(format!(
            "A Bloom capacity sample used {engine_executions} engines instead of two."
        ));
    }
    let mut record = json!({"bloom_hash":check::artifact_hash(&grade)?,"bloomed":grade.bloomed,"modeled_work":modeled_work,"costs":costs,"receipt_ms":receipt_ms,"serialize_ms":serialize_ms,"verify_ms":verify_ms,"total_ms":total_ms,"receipt_bytes":receipt_bytes,"result_hash":result_hash,"engine_executions":engine_executions,"rss_after_release":rss});
...
fn main_result() -> Result<(), String> {
    ...
    if arguments.len() != 2 {
        return Err(
            "Usage: bloom_capacity BLOOM_LEFT_EXPERIMENT BLOOM_CROSSING_RIGHT_EXPERIMENT".into(),
        );
    }
    let workloads = arguments
        .iter()
        .zip(["bloom-left", "bloom-crossing-right"])
        .map(|(file, case_id)| {
            let metadata = std::fs::symlink_metadata(file).map_err(|e| e.to_string())?;
            if !metadata.is_file() || metadata.len() > MAX_INPUT_BYTES as u64 {
                return Err("Capacity input must be a regular file within 64 KiB.".into());
            }
            let opened = std::fs::File::open(file).map_err(|e| e.to_string())?;
            ...
            if experiment != bloom_fixtures::experiment(case_id)? {
                return Err(format!(
                    "Capacity input must be the exact supplied {case_id} world."
                ));
            }
            Ok((case_id, experiment))
        })
    ...
    for (id, experiment) in workloads {
        ...
        eprintln!(
            "{}",
            json!({"schema":"platonik-bloom-capacity-sample-v1","workload":id,"warmup":true,"sample":warmup})
        );
        let samples = (0..30)
            .map(|index| {
                let sample = sample(&experiment, buffer.as_mut())?;
                eprintln!("{}", json!({"schema":"platonik-bloom-capacity-sample-v1","workload":id,"index":index,"warmup":false,"sample":sample}));
    ...
    let mut record = json!({"schema":"platonik-bloom-capacity-v1","scope":"Only the two exact eight-cell v4 Bloom worlds; no larger-world or integrated-campaign admission.","percentile_method":"nearest rank over 30 measured samples; warmup excluded","samples_per_workload":30,"warmups_per_workload":1,"workloads":measured,"engine_executions":sim::execution_count()});
    if buffered {
        record["schema"] = json!("platonik-bloom-capacity-v1");
    ...
    if sim::execution_count() != 124 {
        return Err("Bloom capacity requires exactly 124 recorded engine executions.".into());
    }
    println!("{record}");
    Ok(())
}
