//! Two predeclared arithmetic-and-service inputs, thirty measured repetitions each.
//! Every warmup/sample constructs and independently grades a fresh receipt.
use platonik_core::{ark_control, ark_fixtures, check, model::*, parse_experiment, sim};
...
fn sample(
    ...
    let start = Instant::now();
    let ark = ark_control::grade_receipt(&receipt)?;
    ...
    let rss = current_rss();
    let mut record = json!({"ark_hash":check::artifact_hash(&ark)?,"receipt_ms":receipt_ms,"serialize_ms":serialize_ms,"verify_ms":verify_ms,"total_ms":total_ms,"receipt_bytes":receipt_bytes,"result_hash":result_hash,"engine_executions":sim::execution_count()-before,"rss_after_release":rss});
...
fn main_result() -> Result<(), String> {
    ...
    if arguments.len() != 2 {
        return Err("Usage: ark_capacity FIRST_EXPERIMENT SECOND_EXPERIMENT".into());
    }
    let workloads = arguments
        .iter()
        .zip(["ark-reserve-16", "ark-staggered-15"])
        .map(|(file, case_id)| {
            ...
            if experiment != ark_fixtures::experiment(case_id)? {
                return Err(format!(
    ...
    let mut record = json!({"schema":"platonik-ark-capacity-v1","workloads":measured,"engine_executions":sim::execution_count()});
    if buffered {
        record["schema"] = json!("platonik-ark-capacity-v1");
