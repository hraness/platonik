//! Two predeclared custody-and-acknowledgment inputs, thirty measured repetitions each.
//! Every warmup/sample constructs and independently grades a fresh receipt.
use platonik_core::{check, model::*, parse_experiment, port_commitments, port_fixtures, sim};
...
fn sample(
    ...
    let start = Instant::now();
    let ports = port_commitments::grade_receipt(&receipt)?;
    ...
    let rss = current_rss();
    let mut record = json!({"ports_hash":check::artifact_hash(&ports)?,"receipt_ms":receipt_ms,"serialize_ms":serialize_ms,"verify_ms":verify_ms,"total_ms":total_ms,"receipt_bytes":receipt_bytes,"result_hash":result_hash,"engine_executions":sim::execution_count()-before,"rss_after_release":rss});
...
fn main_result() -> Result<(), String> {
    ...
    if arguments.len() != 2 {
        return Err("Usage: ports_capacity FIRST_EXPERIMENT SECOND_EXPERIMENT".into());
    }
    let workloads = arguments
        .iter()
        .zip(port_fixtures::capacity_ids())
        .map(|(file, case_id)| {
            ...
            if experiment != port_fixtures::experiment(case_id)? {
                return Err(format!(
    ...
    let mut record = json!({"schema":"platonik-ports-capacity-v1","workloads":measured,"engine_executions":sim::execution_count()});
    if buffered {
        record["schema"] = json!("platonik-ports-capacity-v1");
